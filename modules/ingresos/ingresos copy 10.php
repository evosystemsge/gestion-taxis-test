...
if (isset($_POST['accion'])) {
    ...
    if ($accion == 'agregar') {
        ...
        // Agregar este caso al if(isset($_POST['accion']))
elseif ($accion == 'pagar') {
    ...
    try {
        ...
        ");
        $descripcion = "Pago pendiente de " . ($ingreso['conductor_id'] ? 'conductor ID '.$ingreso['conductor_id'] : '');
        $stmt->execute([
            $ingreso['caja_id'], 
            $ingreso_id,
            $monto_pagado,
        ...
        */
        // Calcular monto esperado (valor fijo según tipo)
$monto_esperado = ($tipo_ingreso == 'obligatorio') ? 
($conductor['ingreso_obligatorio'] ?? 0) : 
($conductor['ingreso_libre'] ?? 0);
        ...
        <div class="modal__body">
            <form id="formPagar" method="post" class="modal__form">
                ...
                <div class="modal__form-group">
                    ...
                    <input type="number" name="monto" id="pagarMonto" class="modal__form-input" required>
                </div>
            </form>
        </div>
...
function pagarIngreso(ingresoId, montoPendiente) {
    ...
    // Configuración solo para modalAgregar (agregar/editar)
    ...
    // Esto aplica a TODOS los modales (incluyendo el nuevo modalPagar)
    ...
    function calcularMontoEsperado() {
        ...
        // Buscar conductor en los datos cargados
        ...
        // Monto fijo según tipo de ingreso
...
document.getElementById('monto_esperado').value = montoEsperado;
    ...
    // Función para obtener último ingreso del conductor
