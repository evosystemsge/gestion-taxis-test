...
<head>
    ...
    <style>
    /* ============ ESTILOS PRINCIPALES (COPIADOS DE VEHICULOS.PHP) ============ */
    :root {
...
<body>
    <div class="container">
        ...
        <div class="table-container">
            <table class="table">
                ...
                <tbody id="tableBody">
                    <?php 
                    // Preprocesar los datos para calcular el recorrido
                    $ingresosPorConductor = [];
                    foreach ($ingresos as $ing) {
                        if (!isset($ingresosPorConductor[$ing['conductor_id']])) {
                            $ingresosPorConductor[$ing['conductor_id']] = [];
                        }
                        $ingresosPorConductor[$ing['conductor_id']][] = $ing;
                    }
                    
                    foreach ($ingresos as $index => $ing): 
                        // Calcular recorrido (kilometraje actual - kilometraje anterior)
                        $recorrido = 0;
                        if (isset($ingresosPorConductor[$ing['conductor_id']])) {
                            $ingresosConductor = $ingresosPorConductor[$ing['conductor_id']];
                            $currentIndex = array_search($ing, $ingresosConductor);
                            if ($currentIndex !== false && $currentIndex < count($ingresosConductor) - 1) {
                                $recorrido = $ing['kilometros'] - $ingresosConductor[$currentIndex + 1]['kilometros'];
                            }
                        }
                        
                        // Clase para resaltar pendientes
                    ...
                    ?>
                        <tr data-conductor="<?= $ing['conductor_id'] ?>" 
                            ...
                            <td class="<?= $clasePendiente ?>"><?= number_format($ing['monto_pendiente'], 0, ',', '.') ?> XAF</td>
                            <td><?= $recorrido > 0 ? number_format($recorrido, 0, ',', '.') : '-' ?></td>
                            <td>
                                <?= $ing['ciclo'] ?>
                            ...
                            <td>
                                <div style="display: flex; gap: 5px; flex-wrap: wrap;">
                                    <button class="btn btn-ver" onclick="verIngreso(<?= htmlspecialchars(json_encode($ing), ENT_QUOTES, 'UTF-8') ?>, <?= $recorrido ?>)">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                            <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                                            <circle cx="12" cy="12" r="3"></circle>
                                        </svg>
                                    </button>
                                    <button class="btn btn-editar" onclick="editarIngreso(<?= $ing['id'] ?>)">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="modal__body">
                <form id="formIngreso" method="post" class="modal__form">
                    ...
                    <div class="modal__form-row">
                        ...
                        <div class="modal__form-group">
                            <label for="km_actual" class="modal__form-label">Km Actual Vehículo</label>
                            <input type="number" id="km_actual" class="modal__form-input" readonly>
                        </div>
                    </div>
                </form>
            </div>
    ...
    function initEventListeners() {
        ...
        document.getElementById('openModalAgregar').addEventListener('click', function() {
            abrirModal('modalAgregar', 'agregar');
        });
        ...
        for (let row of rows) {
            ...
            if (rowId == id) {
                return {
                    ...
                    monto_ingresado: parseFloat(row.cells[5].textContent.replace(/\./g, '').replace(' XAF', '')),
    ...
    function cargarKilometrosActuales() {
        ...
        // Obtener el último registro de kilómetros para este conductor
        const table = document.getElementById('tableBody');
        const rows = Array.from(table.getElementsByTagName('tr'));
        ...
        const ingresosConductor = rows
            .filter(row => row.getAttribute('data-conductor') == conductorId)
            .map(row => ({
                kilometros: parseFloat(row.cells[8].textContent)
            }));
        
        if (ingresosConductor.length > 0) {
            document.getElementById('km_actual').value = ingresosConductor[0].kilometros;
        } else {
            document.getElementById('km_actual').value = '';
        }
    }
    ...
    function verIngreso(ingreso, recorrido = 0) {
        currentIngreso = ingreso;
        ...
        const columna2 = [
            ...
            { label: 'RECORRIDO', value: (recorrido > 0 ? numberFormat(recorrido) + ' km' : 'N/A') },
            { label: 'CICLO', value: ingreso.ciclo || 'N/A' }
        ];
    ...
    function editarIngreso(id = null) {
        if (!id && currentIngreso) {
            id = currentIngreso.id;
        }
