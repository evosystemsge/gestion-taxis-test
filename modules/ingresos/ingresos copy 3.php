...
if (isset($_POST['accion'])) {
    ...
    if ($accion == 'agregar') {
        ...
        // Caja predeterminada (suponemos que el ID de la caja predeterminada es 1)
        $caja_id = 1; // Cambiar a la ID de la caja predeterminada

        // Obtener el monto esperado
        $stmt = $pdo->prepare("SELECT * FROM conductores WHERE id = ?");
        $stmt->execute([$conductor_id]);
        $conductor = $stmt->fetch();
        $monto_esperado = $conductor[$tipo_ingreso == 'obligatorio' ? 'ingreso_dia_obligatorio' : 'ingreso_dia_libre'];
        $monto_pendiente = 0;

        // Verificar el último ingreso del conductor
        ...
        // Actualizamos los kilómetros en la tabla vehiculos
