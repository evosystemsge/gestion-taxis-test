...
if (isset($_POST['accion'])) {
    ...
    if ($accion == 'agregar') {
        ...
        // Reemplazar el cálculo del monto esperado y pendiente por esto:
        $monto_esperado = ($tipo_ingreso == 'obligatorio') ? 
        ($conductor['ingreso_obligatorio'] ?? 0) : 
        ($conductor['ingreso_libre'] ?? 0);

        $monto_pendiente = $monto_esperado - $monto_ingresado;
        ...
        try {
            ...
            $stmt->execute([
                $fecha, $conductor_id, $tipo_ingreso, $monto_ingresado, $monto_esperado,
                $monto_pendiente, // Este es simplemente monto_esperado - monto_ingresado
                $kilometros, $recorrido, $caja_id, $nuevo_ciclo, $contador_ciclo, $ciclo_completado
            ]);
...
$ingresos = $pdo->query("
    SELECT i.*, c.nombre AS conductor_nombre, v.marca, v.modelo, v.matricula, v.km_actual, c.dias_por_ciclo, i.ciclo, i.contador_ciclo
    FROM ingresos i
...
<body>
    ...
    // Calcular directamente del conductor sin considerar últimos ingresos
