        ...
        switch ($_GET['action']) {
            case 'obtener_pagos_prestamo':
                ...
                case 'obtener_ingreso':
                    ...
                    $stmt = $pdo->prepare("
                        SELECT i.*, c.nombre AS conductor_nombre, c.ingreso_obligatorio, c.ingreso_libre
                        FROM ingresos i
                    ...
                    exit;
                    case 'verificar_pagos_prestamo':
                        if (!isset($_GET['prestamo_id'])) {
                            throw new Exception('ID de préstamo no especificado');
                        }
                        
                        $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM pagos_prestamos WHERE prestamo_id = ?");
                        $stmt->execute([$_GET['prestamo_id']]);
                        $result = $stmt->fetch(PDO::FETCH_ASSOC);
                        
                        echo json_encode(['tiene_pagos' => $result['total'] > 0]);
                        exit;   
                    ...
                    try {
                        ...
                        switch ($tipo) {
                            case 'prestamo':
                                ...
                                $stmt = $pdo->prepare("SELECT * FROM pagos_prestamos WHERE id = ?");
                                $stmt->execute([$id]);
                                $pago = $stmt->fetch(PDO::FETCH_ASSOC);
                                
                                if ($pago) {
                                    // Revertir el pago en el préstamo
                                    $stmt = $pdo->prepare("UPDATE prestamos SET saldo_pendiente = saldo_pendiente + ? WHERE id = ?");
                                    $stmt->execute([$pago['monto'], $pago['prestamo_id']]);
                                    
                                    // Eliminar movimiento de caja
                                    $stmt = $pdo->prepare("DELETE FROM movimientos_caja WHERE pago_prestamo_id = ?");
                                    $stmt->execute([$id]);
                                    
                                    // Eliminar el pago
                                    $stmt = $pdo->prepare("DELETE FROM pagos_prestamos WHERE id = ?");
                                    $stmt->execute([$id]);
                                }
                                break;
                                
                            // Añadir casos para otros tipos de pagos (amonestacion, ingreso, etc.)
...
<body>
    ...
    <input type="date" id="filterFechaDesde" class="modal__form-input">
</div>
<div class="modal__form-group">
    <label>Fecha hasta:</label>
    <input type="date" id="filterFechaHasta" class="modal__form-input">
</div>
<div class="modal__form-group">
    <label>Monto desde:</label>
    <input type="number" id="filterMontoDesde" class="modal__form-input" min="0" placeholder="Mínimo">
</div>
<div class="modal__form-group">
    <label>Monto hasta:</label>
    <input type="number" id="filterMontoHasta" class="modal__form-input" min="0" placeholder="Máximo">
</div>
    ...
    fetch(`deudas.php?action=obtener_ingreso&id=${ingresoId}`)
        .then(response => response.json())
        .then(data => {
            ...
            // Mostrar monto esperado según tipo de ingreso
            const montoEsperado = data.tipo_ingreso === 'obligatorio' ? 
                data.ingreso_obligatorio : data.ingreso_libre;
            document.getElementById('ingresoMontoEsperado').textContent = montoEsperado + ' XAF';
    ...
    fetch(`deudas.php?action=obtener_amonestacion&id=${amonestacionId}`)
        .then(response => response.json())
        .then(data => {
            ...
            document.getElementById('btnAnularAmonestacionModal').onclick = function() {
                if (confirm('¿Está seguro de anular esta amonestación?')) {
...
function cambiarEstadoAmonestacion(amonestacionId, estado) {
    ...
    formData.append('accion', 'cambiar_estado_amonestacion');
...
function aplicarFiltros() {
    ...
    const montoDesde = document.getElementById('filterMontoDesde').value;
    const montoHasta = document.getElementById('filterMontoHasta').value;
    
    Array.from(tableBody.rows).forEach(row => {
        const rowConductor = row.getAttribute('data-conductor');
        const rowEstado = row.getAttribute('data-estado');
        const rowFecha = row.cells[1].textContent; // Asume que la fecha está en la segunda celda
        const rowMonto = parseFloat(row.cells[3].textContent.replace(/[^0-9.]/g, '')); // Asume que el monto está en la cuarta celda
        
        let mostrar = true;
        
        // Aplicar filtros
        if (conductorId && rowConductor !== conductorId) mostrar = false;
        ...
        if (montoDesde && rowMonto < parseFloat(montoDesde)) mostrar = false;
        if (montoHasta && rowMonto > parseFloat(montoHasta)) mostrar = false;
...
document.getElementById('filterMontoDesde').addEventListener('input', aplicarFiltros);
document.getElementById('filterMontoHasta').addEventListener('input', aplicarFiltros);
function eliminarPago(tipo, pagoId) {
    ...
    function confirmarEliminarPrestamo(form, prestamoId) {
    if (confirm('¿Está seguro de eliminar este préstamo?')) {
        fetch(`deudas.php?action=verificar_pagos_prestamo&prestamo_id=${prestamoId}`)
            .then(response => response.json())
            .then(data => {
                if (data.tiene_pagos) {
                    alert('No se puede eliminar un préstamo con pagos asociados. Elimine primero los pagos.');
                    return false;
                } else {
                    return confirm('¿Confirmas que deseas eliminar este préstamo?');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                alert('Error al verificar pagos');
                return false;
            });
    }
    return false;
